...
def read_csv() -> list:
    employee = []
    with open(Path.cwd() / 'Python_Seminar8\database.csv', 'r', encoding='utf-8') as fin:
...
def read_json() -> list:
    employee = []
    with open(Path.cwd() / 'Python_Seminar8\database02.json', 'r', encoding='utf-8') as fin:
        for line in fin:
...
def write_csv(employees: list):
    with open(Path.cwd() / 'Python_Seminar8\database.csv', 'w', encoding='utf-8') as fout:
        csv_writer = csv.writer(fout)
        for employee in employees:
            csv_writer.writerow(employee.values())

def write_json(employees: list):
    with open(Path.cwd() / 'Python_Seminar8\database02.json', 'w', encoding='utf-8') as fout:
        for employee in employees:
